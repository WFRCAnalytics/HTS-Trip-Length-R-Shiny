...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    selectInput("dataSource", "Display:",
                 choices = setNames(labelsDisplay$value, labelsDisplay$label),
                 selected = "lengths"),
...
server <- function(input, output, session) {
  ...
  selectedData <- reactive({
    ...
    data <- dataSources[[input$dataSource]]
    data
  })
  ...
  observe({
    ...
    output$dataPlot <- renderPlot({
      ...
      data <- selectedData() %>%
        ...
        # Filter out rows with NA values in relevant columns to avoid warnings
        filter(!is.na(binSize), !is.na(pctTripWeight), !is.na(numTripRecords), !is.na(cumPctTripWeight))
      
      lowerlimit <- -0.5 * numeric_binSize
      
      # Set x-axis label dynamically based on dataSource
      x_axis_label <- if (input$dataSource == "lengths") "Distance (miles)" else "Duration (minutes)"
      
      if (input$typeChart == "histogram") {
        ggplot(data, aes(x = binStart + numeric_binSize / 2, y = pctTripWeight)) +
        ...
        ggplot(data, aes(x = binStart + numeric_binSize / 2, y = numTripRecords)) +
          geom_bar(stat = "identity", position = "dodge", fill = "steelblue", width = numeric_binSize) +
          scale_x_continuous(limits = c(lowerlimit, numeric_maxX + numeric_binSize / 2),
                             breaks = seq(0, numeric_maxX, by = 5)) +
        ...
        ggplot(data, aes(x = binStart, y = cumPctTripWeight)) +
          geom_line() +
